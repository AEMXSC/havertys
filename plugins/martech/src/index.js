  ...
  if (config.analytics) {
    ...
    window[config.dataLayerInstanceName].push((dl) => {
      dl.addEventListener('adobeDataLayer:event', (payload) => {
        ...
        const args = [{ eventType, ...payload.xdm }, payload.data, payload.configOverrides];
...
async function applyPropositions(instanceName) {
  ...
  onDecoratedElement(async () => {
    ...
    const appliedPropositions = await window[instanceName]('applyPropositions', { propositions });
  ...
  alloyConfig = {
    ...
    onBeforeEventSend: (payload) => {
      ...
      if (config.includeDataLayerState) {
        const dlState = window.adobeDataLayer.getState ? window.adobeDataLayer.getState() : window.adobeDataLayer[0];
        payload.xdm = {
          ...payload.xdm,
          ...dlState,
        };
