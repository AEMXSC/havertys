export default function decorate(block) {
  ...
  imageContainer.classList.add('commerce-teaser-video-image-container');

  const contentContainer = document.createElement('div');
  contentContainer.classList.add('commerce-teaser-video-content-container');

  rows.forEach((row) => {
    const cols = [...row.querySelectorAll(':scope > div')];
    cols.forEach((col) => {
      const hasImage = col.querySelector('picture, img, video');
      if (hasImage && !contentContainer.querySelector('.commerce-teaser-video-titles')) {
        imageContainer.append(col);
      } else {
        const titles = document.createElement('div');
        titles.classList.add('commerce-teaser-video-titles');

        const description = document.createElement('div');
        description.classList.add('commerce-teaser-video-description');
